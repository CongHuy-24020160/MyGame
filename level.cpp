...
void Level::loadMap(std::string mapName, Graphics& graphics) {
	...
	XMLDocument doc;
	std::stringstream ss;
	ss << "contents/Maps/" << mapName << ".tmx"; //Pass in Map 1, we get maps/Map 1.tmx
	//doc.LoadFile(ss.str().c_str());
	if (doc.LoadFile(ss.str().c_str()) != tinyxml2::XML_SUCCESS) {
		std::cout << "Lỗi khi đọc file TMX!" << std::endl;
	}
	...
	if (pTileset != NULL) {
		while (pTileset) {
			...
			if (graphics.getRenderer() == NULL) {
				std::cout << "Error in render";
			}

			if (!tex) {
				std::cout << "Error in source path";
			}
